...
mod tests {
    ...
    async fn transcode_process_exposes_linux_resource_samples() {
        // The sampler ticks every TRANSCODE_RESOURCE_SAMPLE_INTERVAL, and its first tick can read
        // the child before the kernel reports any RSS for it. Outlive one further tick so the wait
        // below cannot expire on that ordering.
        let command = FfmpegCommandSpec::new("sh", vec!["-c".to_string(), "sleep 3".to_string()]);
        let mut process = spawn_transcode_process(&command).unwrap();
        ...
        let sample = timeout(
            crate::TRANSCODE_RESOURCE_SAMPLE_INTERVAL + Duration::from_secs(1),
            async {
                loop {
