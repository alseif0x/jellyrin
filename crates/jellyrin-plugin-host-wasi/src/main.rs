...
async fn handle_envelope(
    ...
    match envelope.method {
        ...
        | PluginRpcMethod::GetEmbeddedImage => failure(
            ...
            "WASI host metadata-only milestone does not implement this method yet.",
        ),
    }
}
...
fn handle_invoke_capability(
    ...
    success(
        ...
        CapabilityResult {
            value: json!({
                "Status": "NotExecuted",
                "Reason": "WASI execution engine is not wired in this milestone."
            }),
        },
    )
}
...
mod tests {
    ...
    #[tokio::test]
    async fn wasi_host_rejects_missing_wasm_and_unsupported_method() {
        let temp = tempdir().unwrap();
        let plugin_dir = temp.path().join("plugin");
        tokio::fs::create_dir_all(&plugin_dir).await.unwrap();
