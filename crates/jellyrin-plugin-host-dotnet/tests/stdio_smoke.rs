...
#[tokio::test]
async fn dotnet_host_binary_round_trips_over_stdio() {
    ...
    let load = PluginRpcEnvelope::new(
        ...
        LoadPluginRequest {
            plugin_id: "stdio-dotnet-fixture".to_string(),
            name: "Stdio DotNet Fixture".to_string(),
            version: "1.0.0.0".to_string(),
            runtime: PluginRuntime::DotNetJellyfin,
            target_abi: "12.0.0.0".to_string(),
            install_path: plugin_dir.to_string_lossy().to_string(),
            manifest: json!({
                "Name": "Stdio DotNet Fixture",
                "Capabilities": ["MetadataProvider", "ImageProvider"]
            }),
            permissions: Vec::new(),
        },
    );
    ...
    assert_eq!(loaded.plugin_id, "stdio-dotnet-fixture");
    assert_eq!(loaded.runtime, PluginRuntime::DotNetJellyfin);
    assert_eq!(loaded.capabilities, ["MetadataProvider", "ImageProvider"]);
    ...
    assert_eq!(health.plugin_id, "stdio-dotnet-fixture");
    assert_eq!(health.runtime, PluginRuntime::DotNetJellyfin);
...
fn host_binary_path() -> PathBuf {
    std::env::var_os("CARGO_BIN_EXE_jellyrin-plugin-host-dotnet")
        .map(PathBuf::from)
        .unwrap_or_else(|| {
