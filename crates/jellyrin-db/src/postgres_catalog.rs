...
pub async fn ensure_media_item_query_filter_projection(
    ...
    .execute(&mut **tx)
    .await?;
    let folder_ids = sqlx::query_scalar::<_, Uuid>("SELECT id FROM virtual_folders ORDER BY id")
        .fetch_all(&mut **tx)
        .await?;
    for folder_id in folder_ids {
        PostgresDatabase::rebuild_media_item_query_filter_summary_in_transaction(tx, folder_id)
            .await?;
    }
...
impl PostgresDatabase {
    ...
    async fn media_item_query_filter_values_unobserved(
        ...
        if let Some((folder_ids, effective_item_types)) = postgres_query_filter_summary_scope(query)
            && let Some(values) = Self::media_item_query_filter_values_summary(
                &folder_ids,
    ...
    async fn media_item_query_filter_values_summary(
        folder_ids: &[Uuid],
        ...
        let rows = sqlx::query_as::<_, PostgresQueryFilterValueRow>(
            r#"
            WITH ranked AS (
                SELECT value_kind AS kind, normalized_value, display_value,
                       row_number() OVER (
                           PARTITION BY value_kind, normalized_value
                           ORDER BY winner_item_sort COLLATE "C", winner_item_id,
                                    winner_source_priority, winner_source_position
                       ) AS spelling_rank
                FROM media_item_query_filter_summary_values
                WHERE virtual_folder_id = ANY($1)
                  AND effective_item_type = ANY($2)
                  AND value_kind = ANY($3)
            )
        ...
        .bind(selection.summary_fields())
        .fetch_all(&mut *connection)
        .await?;
        Ok(Some(postgres_query_filter_values_from_rows(rows)?))
    }
    ...
    async fn rebuild_media_item_query_filter_summary_in_transaction(
        tx: &mut Transaction<'_, Postgres>,
    ...
    async fn lock_media_item_query_filter_summary_in_transaction(
        tx: &mut Transaction<'_, Postgres>,
        virtual_folder_id: Uuid,
    ) -> anyhow::Result<()> {
        sqlx::query(
            "SELECT pg_advisory_xact_lock(hashtextextended(\
                 'jellyrin-query-filter-summary:' || $1::text, 0))",
        )
        .bind(virtual_folder_id)
        .execute(&mut **tx)
        .await?;
    ...
    async fn begin_media_item_query_filter_summary_point_update(
        tx: &mut Transaction<'_, Postgres>,
        item_id: Uuid,
    ) -> anyhow::Result<MediaItemQueryFilterSummaryPointContext> {
        let locked_folder_id = sqlx::query_scalar::<_, Uuid>(
            "SELECT virtual_folder_id FROM media_items WHERE id = $1",
        )
        .bind(item_id)
        .fetch_one(&mut **tx)
        .await?;
        ...
        let (folder_id, effective_item_type, item_sort, path, media_type, streams, metadata) = item
            ...
            .await?;
        anyhow::ensure!(
            folder_id == locked_folder_id,
            "media item moved while acquiring query-filter summary lock"
        );
        let stream_values = streams.as_array().map(Vec::as_slice).unwrap_or(&[]);
        let old_projection =
    ...
    async fn replace_remote_media_library_snapshot_in_transaction(
        ...
        Self::lock_media_item_query_filter_summary_in_transaction(tx, folder_id).await?;
        let sync_run_id = Uuid::new_v4();
    ...
    async fn publish_prepared_remote_media_library_stage_in_transaction(
        ...
        let summary_current = sqlx::query_scalar::<_, bool>(
            "SELECT EXISTS (\
             SELECT 1 \
             FROM media_item_query_filter_summary_coverage AS coverage \
             JOIN media_item_query_filter_summary_revisions AS revision \
...
/// Return the exact summary domain for the common Jellyfin filter shape.
///
/// The summary currently covers remote Movie and Episode libraries. Every additional predicate
/// must keep using the item-level projection so a cache-like optimization can never broaden the
/// selected set. A user id alone is harmless because playback state is not consulted unless one
/// of the user-state predicates below is active.
fn postgres_query_filter_summary_scope(
    ...
    let mut folder_ids = query.virtual_folder_ids.clone();
    folder_ids.sort_unstable();
    folder_ids.dedup();
    Some((folder_ids, effective_item_types))
}
...
mod tests {
    ...
    async fn postgres_movie_filter_values_benchmark() {
        ...
        let result = async {
            ...
            let query = MediaItemCatalogQuery {
                limit: 0,
                virtual_folder_ids: vec![movie_folder.id],
                include_item_types: vec!["Movie".to_string()],
                ..MediaItemCatalogQuery::default()
            };
            for _ in 0..2 {
    ...
    async fn postgres_query_filter_values_are_unpaged_scoped_and_exact() {
        ...
        let result = async {
            ...
            let summary_coverage: i64 = sqlx::query_scalar(
                "SELECT count(*) FROM media_item_query_filter_summary_coverage \
                 WHERE virtual_folder_id = $1 \
                   AND effective_item_type = 'movie' \
            ...
            .fetch_one(&test.database.pool)
            .await?;
            // Conflicting display spellings keep the exact item-order winner in the summary;
            // they must not disable acceleration for the whole folder.
            assert_eq!(summary_coverage, 1);
            ...
            let summary_folder = test
                ...
                .replace_remote_media_library_snapshot(
                    "Summary Filters",
                    "movies",
                    "provider://summary-filters",
                    vec![remote_item(
                        summary_item_id,
                        "Summary Movie",
                        "provider://summary-filters/movie.mkv",
                        "Video",
                        "movies",
                        json!({ "Genres": ["Summary Genre"], "Tags": ["Summary Tag"] }),
                    )],
                )
                .await?;
            let summary_query = MediaItemCatalogQuery {
                limit: 0,
                virtual_folder_ids: vec![summary_folder.id],
                include_item_types: vec!["Movie".to_string()],
                ..MediaItemCatalogQuery::default()
            };
            let summary_result = test
                .database
                .media_item_query_filter_values(&summary_query, MediaItemQueryFilterSelection::ALL)
                .await?;
            assert_eq!(summary_result.genres, ["Summary Genre"]);
            let fast_coverage: i64 = sqlx::query_scalar(
                "SELECT count(*) FROM media_item_query_filter_summary_coverage \
                 WHERE virtual_folder_id = $1 AND effective_item_type = 'movie'",
            )
            .bind(summary_folder.id)
            .fetch_one(&test.database.pool)
            .await?;
            assert_eq!(fast_coverage, 1);
            ...
            .execute(&test.database.pool)
            .await?;
            let projected_result = test
                .database
                .media_item_query_filter_values(&summary_query, MediaItemQueryFilterSelection::ALL)
                .await?;
            assert_eq!(summary_result, projected_result);

            // Restore coverage with an unchanged publication first, so the next changed
            // publication proves that an existing current marker cannot hide stale values.
            test.database
                .replace_remote_media_library_snapshot(
                    "Summary Filters",
                    "movies",
                ...
                .await?;
            let restored_coverage: i64 = sqlx::query_scalar(
                "SELECT count(*) FROM media_item_query_filter_summary_coverage \
                 WHERE virtual_folder_id = $1 AND effective_item_type = 'movie'",
            )
            .bind(summary_folder.id)
            .fetch_one(&test.database.pool)
            .await?;
            assert_eq!(restored_coverage, 1);

            // A publication with changed metadata must invalidate and atomically rebuild an
            // already-covered summary. Otherwise the fast path could return stale filters.
            test.database
                .replace_remote_media_library_snapshot(
                    "Summary Filters",
                    "movies",
                    ...
                    vec![remote_item(
                        ...
                        "Video",
                        "movies",
                        json!({ "Genres": ["Updated Genre"], "Tags": ["Updated Tag"] }),
                    )],
                )
                .await?;
            let updated_summary = test
                .database
                .media_item_query_filter_values(&summary_query, MediaItemQueryFilterSelection::ALL)
                .await?;
            assert_eq!(updated_summary.genres, ["Updated Genre"]);
            assert_eq!(updated_summary.tags, ["Updated Tag"]);
            let rebuilt_coverage: i64 = sqlx::query_scalar(
                "SELECT count(*) FROM media_item_query_filter_summary_coverage \
                 WHERE virtual_folder_id = $1 AND effective_item_type = 'movie'",
            )
            .bind(summary_folder.id)
            .fetch_one(&test.database.pool)
            .await?;
            assert_eq!(rebuilt_coverage, 1);

            // Detail/playback probes update one Xtream item at a time. They must patch the
            // covered summary atomically instead of forcing a full-library rebuild or leaving
            // the fast path disabled until the next snapshot publication.
            test.database
                .update_media_item_metadata(
                    summary_item_id,
                    json!({ "Genres": ["Point Genre"], "Tags": ["Point Tag"] }),
                )
                .await?;
            let point_metadata_summary = test
                .database
                .media_item_query_filter_values(&summary_query, MediaItemQueryFilterSelection::ALL)
                .await?;
            assert_eq!(point_metadata_summary.genres, ["Point Genre"]);
            assert_eq!(point_metadata_summary.tags, ["Point Tag"]);
            let revision_after_metadata = sqlx::query_as::<_, (i64, Option<i64>, i64)>(
            ...
            .fetch_one(&test.database.pool)
            .await?;
            assert_eq!(revision_after_metadata.1, Some(revision_after_metadata.0));
            assert_eq!(revision_after_metadata.2, revision_after_metadata.0);
            test.database
                .update_media_item_metadata(
                    summary_item_id,
                    json!({ "Genres": ["Point Genre"], "Tags": ["Point Tag"] }),
                ...
                .await?;
            let revision_after_metadata_noop = sqlx::query_as::<_, (i64, Option<i64>, i64)>(
                "SELECT revision.source_revision, revision.reconciled_revision, \
                        coverage.source_revision \
                 FROM media_item_query_filter_summary_revisions AS revision \
                 JOIN media_item_query_filter_summary_coverage AS coverage \
            ...
            .fetch_one(&test.database.pool)
            .await?;
            assert_eq!(revision_after_metadata_noop, revision_after_metadata);
            ...
            test.database
                .update_media_item_media_info(
                    ...
                    None,
                    None,
                    vec![json!({ "Type": "Subtitle", "Language": "spa" })],
                )
                .await?;
            let point_stream_summary = test
                .database
                .media_item_query_filter_values(&summary_query, MediaItemQueryFilterSelection::ALL)
                .await?;
            assert_eq!(point_stream_summary.genres, ["Point Genre"]);
            assert_eq!(point_stream_summary.subtitle_languages, ["spa"]);
            assert!(point_stream_summary.has_subtitles);
            let point_coverage: i64 = sqlx::query_scalar(
                "SELECT count(*) FROM media_item_query_filter_summary_coverage \
                 WHERE virtual_folder_id = $1 AND effective_item_type = 'movie'",
            )
            .bind(summary_folder.id)
            ...
            let case_item = |id, name: &str, genre: &str| {
                remote_item(
                    id,
                    name,
                    &format!("provider://case-filters/{id}.mkv"),
                    "Video",
                    "movies",
                    json!({ "Genres": [genre] }),
                )
            };
            let case_folder = test
                .database
                .replace_remote_media_library_snapshot(
                    "Case Filters",
                    "movies",
                    "provider://case-filters",
                    vec![
                        case_item(case_first_id, "A Winner", "Drama"),
                        case_item(case_second_id, "B Contributor", "drama"),
                    ],
                )
                .await?;
            let case_query = MediaItemCatalogQuery {
                limit: 0,
                virtual_folder_ids: vec![case_folder.id],
                include_item_types: vec!["Movie".to_owned()],
                ..MediaItemCatalogQuery::default()
            };
            test.database
                .update_media_item_metadata(case_first_id, json!({ "Genres": ["drama"] }))
                .await?;
            let normalized_case = test
                .database
                .media_item_query_filter_values(&case_query, MediaItemQueryFilterSelection::ALL)
                .await?;
            assert_eq!(normalized_case.genres, ["drama"]);
            let normalized_contributors: i64 = sqlx::query_scalar(
                "SELECT contributor_count \
                 FROM media_item_query_filter_summary_values \
                 WHERE virtual_folder_id = $1 AND effective_item_type = 'movie' \
                   AND value_kind = 'genres' AND normalized_value = 'drama'",
            )
            .bind(case_folder.id)
            .fetch_one(&test.database.pool)
            .await?;
            assert_eq!(normalized_contributors, 2);

            // A writer that does not participate in the advisory-lock protocol may advance the
            // revision between capture and publication. The point writer must detect that CAS
            // mismatch and rebuild exactly; it cannot adopt the other writer's revision.
            let mut point_tx = test.database.pool.begin().await?;
            let point_context =
                PostgresDatabase::begin_media_item_query_filter_summary_point_update(
                    &mut point_tx,
                    case_first_id,
                )
                .await?;
            let mut concurrent_tx = test.database.pool.begin().await?;
            let concurrent_metadata = json!({ "Genres": ["Concurrent Genre"] });
            sqlx::query("UPDATE media_items SET metadata = $2 WHERE id = $1")
                .bind(case_second_id)
                .bind(&concurrent_metadata)
                .execute(&mut *concurrent_tx)
                .await?;
            replace_postgres_media_item_facets(
                &mut concurrent_tx,
                case_second_id,
                &concurrent_metadata,
            )
            .await?;
            replace_postgres_media_item_query_filter_projection_from_live(
                &mut concurrent_tx,
                case_second_id,
            )
            .await?;
            concurrent_tx.commit().await?;

            let point_metadata = json!({ "Genres": ["Point CAS Genre"] });
            sqlx::query("UPDATE media_items SET metadata = $2 WHERE id = $1")
                .bind(case_first_id)
                .bind(&point_metadata)
                .execute(&mut *point_tx)
                .await?;
            ...
            .await?;
            point_tx.commit().await?;
            let concurrent_summary = test
                .database
                .media_item_query_filter_values(&case_query, MediaItemQueryFilterSelection::ALL)
                .await?;
            assert_eq!(
                concurrent_summary
                    ...
                    .into_iter()
                    .collect::<BTreeSet<_>>(),
                BTreeSet::from(["Concurrent Genre".to_owned(), "Point CAS Genre".to_owned()])
            );
            let concurrent_revision_current: bool = sqlx::query_scalar(
                "SELECT EXISTS (\
                 SELECT 1 \
                 FROM media_item_query_filter_summary_coverage AS coverage \
                 JOIN media_item_query_filter_summary_revisions AS revision \
            ...
            .fetch_one(&test.database.pool)
            .await?;
            assert!(concurrent_revision_current);
                ...
                .replace_remote_media_library_snapshot(
                    ...
                    vec![
                        ...
                        remote_item(
                            tombstone_id,
                            "Removed Movie",
                            "provider://summary-filters/removed.mkv",
                            "Video",
                            "movies",
                            json!({ "Genres": ["Removed Genre"] }),
                        ),
                    ],
                )
                .await?;
            test.database
                ...
                .await?;
            sqlx::query("DELETE FROM media_item_query_filter_sources WHERE item_id = $1")
                .bind(summary_item_id)
                .execute(&test.database.pool)
                .await?;
            let mut corrupted_tx = test.database.worker_pool.begin().await?;
            PostgresDatabase::rebuild_media_item_query_filter_summary_in_transaction(
                &mut corrupted_tx,
                summary_folder.id,
            )
            .await?;
            corrupted_tx.commit().await?;
            let compensated_coverage: i64 = sqlx::query_scalar(
                "SELECT count(*) FROM media_item_query_filter_summary_coverage \
                 WHERE virtual_folder_id = $1",
            )
            .bind(summary_folder.id)
            .fetch_one(&test.database.pool)
            .await?;
            assert_eq!(compensated_coverage, 0);
            ...
            .fetch_one(&test.database.pool)
            .await?;
            assert_eq!(invalidated_coverage, 0);
