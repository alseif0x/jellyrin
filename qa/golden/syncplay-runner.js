...
const outputRoot = process.env.JELLYRIN_SYNCPLAY_RUNNER_TMP
  || path.join(os.tmpdir(), 'jellyrin-syncplay-');
const adminPassword = process.env.JELLYRIN_SYNCPLAY_ADMIN_PASSWORD || 'e4-syncplay-secret';

async function main() {
  await assertFile(upstreamDll, 'upstream Jellyfin DLL');
  await assertFile(upstreamWebDir, 'Jellyfin web dir');
  await assertFile(upstreamFfmpeg, 'Jellyfin ffmpeg');
  await assertFile(jellyrinServer, 'Jellyrin server binary');
  ...
  try {
    ...
    children.push(spawn(jellyrinServer, [
      ...
      JELLYRIN_SYNCPLAY_STALE_TIMEOUT_MS: process.env.JELLYRIN_SYNCPLAY_STALE_TIMEOUT_MS || '3000',
    })));
    ...
    await completeWizard(upstreamUrl, 'E4 Upstream', 'e4upstream');
    await completeWizard(jellyrinUrl, 'E4 Jellyrin', 'e4admin');
    await Promise.all([
      waitForCompletedTarget(upstreamUrl, 'upstream'),
      waitForCompletedTarget(jellyrinUrl, 'jellyrin'),
    ]);

    await runSyncPlayGolden(upstreamUrl, jellyrinUrl);
  } finally {
    await Promise.all(children.map(stopChild));
    if (!process.env.JELLYRIN_KEEP_SYNCPLAY_RUNNER_TMP) {
      await fs.rm(root, { recursive: true, force: true }).catch(() => {});
    } else {
      console.error(`kept syncplay runner temp dir: ${root}`);
    }
  }
}
...
async function runSyncPlayGolden(upstreamUrl, jellyrinUrl) {
  await new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [path.join(repoRoot, 'qa/golden/syncplay-advanced.js')], {
      cwd: repoRoot,
      stdio: 'inherit',
      env: {
        ...process.env,
        ...
        JELLYFIN_ADMIN_USER: 'e4upstream',
        JELLYFIN_ADMIN_PASSWORD: adminPassword,
        JELLYRIN_ADMIN_USER: 'e4admin',
    ...
    child.on('exit', (code, signal) => {
      if (code === 0) {
        ...
        reject(new Error(`syncplay golden exited with ${signal || code}`));
      }
    });
  });
}
