...
const outputRoot = process.env.JELLYRIN_LIVETV_RUNNER_TMP
  || path.join(os.tmpdir(), 'jellyrin-livetv-');
const adminPassword = process.env.JELLYRIN_LIVETV_ADMIN_PASSWORD || 'e2-livetv-secret';
...
async function main() {
  ...
  try {
    ...
    await completeWizard(upstreamUrl, 'E2 Upstream', 'e2upstream');
    await completeWizard(jellyrinUrl, 'E2 Jellyrin', 'e2admin');
    await Promise.all([
      waitForCompletedTarget(upstreamUrl, 'upstream'),
      waitForCompletedTarget(jellyrinUrl, 'jellyrin'),
    ]);

    await runLiveTvGolden(upstreamUrl, jellyrinUrl);
  } finally {
    await Promise.all(children.map(stopChild));
    if (!process.env.JELLYRIN_KEEP_LIVETV_RUNNER_TMP) {
      await fs.rm(root, { recursive: true, force: true }).catch(() => {});
    } else {
      console.error(`kept Live TV runner temp dir: ${root}`);
    }
  }
}
...
async function runLiveTvGolden(upstreamUrl, jellyrinUrl) {
  await new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [path.join(repoRoot, 'qa/golden/livetv-real.js')], {
      cwd: repoRoot,
      stdio: 'inherit',
      env: {
        ...process.env,
        ...
        JELLYFIN_ADMIN_USER: 'e2upstream',
        JELLYFIN_ADMIN_PASSWORD: adminPassword,
        JELLYRIN_ADMIN_USER: 'e2admin',
    ...
    child.on('exit', (code, signal) => {
      if (code === 0) {
        ...
        reject(new Error(`Live TV golden exited with ${signal || code}`));
      }
    });
  });
}
