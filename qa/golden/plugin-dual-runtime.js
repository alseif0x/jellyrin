...
async function main() {
  ...
  const installedRuntimeExecutionTestResult = await runCommand('cargo', [
    'test',
    '-p',
    'jellyrin-api',
    'package_installed_rust_wasi_plugin_executes_scheduled_task',
    '--',
    '--nocapture',
  ]);
  ...
  const rustWasiMetadataProviderTestResult = await runCommand('cargo', [
    'test',
    '-p',
    'jellyrin-api',
    'rust_wasi_metadata_provider_feeds_remote_search',
    '--',
    '--nocapture',
  ]);
  const dotNetMetadataProviderTestResult = await runCommand('cargo', [
    'test',
    '-p',
    'jellyrin-api',
    'dotnet_metadata_provider_feeds_remote_search',
    '--',
    '--nocapture',
  ]);
  const rustWasiImageProviderTestResult = await runCommand('cargo', [
    'test',
    '-p',
    'jellyrin-api',
    'rust_wasi_image_provider_feeds_remote_images_and_download',
    '--',
    '--nocapture',
  ]);
  const dotNetImageProviderTestResult = await runCommand('cargo', [
    'test',
    '-p',
    'jellyrin-api',
    'dotnet_image_provider_feeds_remote_images_and_download',
    '--',
    '--nocapture',
  ]);
  ...
  const passed =
    ...
    installedRuntimeExecutionTestResult.code === 0 &&
    rustWasiChannelProviderTestResult.code === 0 &&
    rustWasiMetadataProviderTestResult.code === 0 &&
    dotNetMetadataProviderTestResult.code === 0 &&
    rustWasiImageProviderTestResult.code === 0 &&
    dotNetImageProviderTestResult.code === 0 &&
  ...
  const evidence = {
    ...
    validatedCommands: [
      ...
      'cargo test -p jellyrin-api package_installed_rust_wasi_plugin_executes_scheduled_task -- --nocapture',
      'cargo test -p jellyrin-api rust_wasi_channel_provider_feeds_channels_api -- --nocapture',
      'cargo test -p jellyrin-api rust_wasi_metadata_provider_feeds_remote_search -- --nocapture',
      'cargo test -p jellyrin-api dotnet_metadata_provider_feeds_remote_search -- --nocapture',
      'cargo test -p jellyrin-api rust_wasi_image_provider_feeds_remote_images_and_download -- --nocapture',
      'cargo test -p jellyrin-api dotnet_image_provider_feeds_remote_images_and_download -- --nocapture',
    ...
    openRisks: [
      ...
      'RustWasi host executes manifest-declared i32 WASM exports with zero or one i32 argument and an initial SDK-style host import fixture; full WASI imports, memory/string ABI and production SDK runtime calls are still pending.',
  ...
  await fs.writeFile(
    ...
    renderMarkdown(evidence, [
      ...
      installedRuntimeExecutionTestResult,
      rustWasiChannelProviderTestResult,
      rustWasiMetadataProviderTestResult,
      dotNetMetadataProviderTestResult,
      rustWasiImageProviderTestResult,
      dotNetImageProviderTestResult,
