...
const goldenCommands = [
  ...
  [
    ...
    '-p',
    'jellyrin-api',
    'dlna_search_criteria_supports_metadata_vendor_fields',
    '--',
    '--nocapture',
  ],
  [
  ...
  [
    ...
    '-p',
    'jellyrin-api',
    'dlna_subtitle_streams_advertise_convertible_text_only',
    '--',
    '--nocapture',
  ],
  [
  ...
  [
    'npm',
    'run',
    'qa:packaging-release',
  ],
  [
    'npm',
    'run',
    'golden:dlna:device-fixture',
    '--',
    '--self-test',
  ],
  [
...
function buildEvidence(result, discoveryEvidence, eventsEvidence, manualEvidence) {
  ...
  return {
    ...
    sourcePhase: passed
      ? `E3.1a/E3.1b${discoveryPassed ? '/E3.1c' : ''}/E3.1d/E3.1e/E3.2a/E3.2b/E3.2c/E3.2d/E3.2e/E3.2f${eventsPassed ? '/E3.2g' : ''}/E3.2h/E3.2i/E3.2j/E3.3a/E3.3b/E3.3c/E3.3d/E3.3e/E3.3f/E3.3g/E3.3h/E3.3i/E3.3j/E3.3k/E3.3l/E3.4b/E3.4c/E3.4d/E3.4e/E3.4f/E3.4g/E3.4h/E3.4i/E3.4j/E3.4k/E3.4l/E3.4m/E3.5a/E3.5b/E3.5c/E3.5d/E3.6a/E3.6b/E3.6c/E3.7a/E3.7b/E3.7d/E3.7e/E3.7f/E3.7g/E3.7h/E3.7i/E3.7j/E3.7k/E3.7l/E3.7m/E3.7n/E3.7o/E3.7p/E3.7q/E3.7r/E3.7s/E3.7t/E3.7u/E3.7v/E3.7w/E3.7x/E3.7y/E3.7z/E3.7aa/E3.7ab/E3.7ac/E3.7ad/E3.7ae/E3.7af/E3.7ag/E3.7ah/E3.7ai`
    ...
    evidence: passed
      ? [
          ...
          'Existing focused coverage also verifies descriptors/SCPD with multi-size PNG iconList, MediaReceiverRegistrar SOAP with default-open behavior plus opt-in allow/deny policy by DeviceID, renderer family and peer IP/CIDR, GENA subscribe/renew/unsubscribe, initial/follow-up NOTIFY, persisted SystemUpdateID restore/update state across SQLite reopen, SOAP Browse/Search/GetProtocolInfo/PrepareForConnection/ConnectionComplete, advanced SearchCriteria handling including boolean, not, relational and common metadata/vendor fields plus actor/director/producer/author/publisher/rating/description aliases, SortCriteria handling, UPnP SOAP faults, DIDL root/folders/items/music-album-artist-series-season-containers/thumbnails/subtitles with tokenless DLNA VTT stream routes, convertible text subtitle codec advertising with unsupported graphical subtitles excluded, SSDP PublishedServerUriBySubnet LOCATION selection, configurable SSDP multicast joins from LocalNetworkAddresses, Jellyfin-like local artwork resolution, generated video-frame thumbnails, non-contradictory albumArtURI metadata, DLNA thumbnail profile metadata/contentFeatures, MaxWidth/MaxHeight thumbnail sizing negotiation through ffmpeg, DLNA image headers, generic profile hints, renderer header detection for VLC/Samsung/LG/Sony, VLC HLS-first and Samsung/direct-first DIDL resource ordering, metadata-gated AVC MP4 AAC DLNA.ORG_PN for Samsung/LG/Sony, conservative generic video MIME mapping without invented video DLNA.ORG_PN values, direct stream URLs, HLS fallback route contracts, Docker host-network DLNA packaging guidance and systemd restart/state-directory packaging invariants.',
          'Manual device evidence schema v3 requires the renderer/control-point run to prove iconList, thumbnail and tokenless subtitle route fetches in addition to discovery, Browse and playback; npm run golden:dlna now runs the device fixture, preflight, evidence-schema and runner self-tests so the local gate covers the complete manual-evidence tooling path. npm run golden:dlna:device-fixture creates a reproducible video/audio/subtitle fixture for that device test and includes a real ffmpeg/ffprobe --self-test, npm run golden:dlna:device-preflight prepares a server-side draft with strict argument parsing, requires a private LAN IPv4 base URL instead of loopback/public/hostname targets, and includes a local --self-test, npm run golden:dlna:device-evidence validates private LAN IPv4 server/device addresses, requires network.deviceIp to differ from network.serverIp, requires both jellyrinBaseUrl and SSDP LOCATION IPv4 hosts to be LAN-reachable and to match network.serverIp, requires the SSDP LOCATION path to reference server.serverId, validates testedAt is not future/stale, validates only enumerated device/artifact types with matching file extensions and non-empty local artifacts whose resolved real paths stay under the manual evidence artifacts directory, and requires evidence.server.commit to match current git HEAD. npm run golden:dlna:device-runner can be run from the real LAN control-point host after playback to perform SSDP/descriptor/Browse/Search/media/HLS probes, verify the fetched root descriptor UDN matches server.serverId and write a validator-ready evidence JSON under the manual DLNA evidence directory. The runner rejects --output outside that directory, nested below it or without a .json extension so the generated evidence is counted by npm run golden:dlna. The runner artifact records raw SSDP plus descriptor byte length, SHA-256 and descriptor invariant booleans for later audit. The runner includes a local --self-test for argument parsing, URL normalization, SSDP header parsing, descriptor digest/UDN assertion, output path containment, mandatory SSDP LOCATION enforcement, SOAP ObjectID escaping and HLS relative playlist/segment URL resolution.',
          deviceValidated
    ...
    openRisks: deviceValidated ? [] : [
      ...
      `Run npm run golden:dlna:device-fixture, add the generated fixture directory as a movies library, run npm run golden:dlna:device-preflight -- --base-url http://<server-lan-ip>:8097 --item-id <media-item-id> --subtitle-index 2, then run npm run golden:dlna:device-runner from the real LAN control-point host after playback to write at least one passing DLNA device evidence JSON under ${deviceEvidence.directory}.`,
      ...
      'ContentDirectory Search supports a broad practical subset of criteria including boolean, not, relational operators and common metadata/vendor aliases; obscure renderer-specific SearchCriteria fields may still require device traces.',
      'Thumbnails advertise DLNA profile metadata/contentFeatures, support bounded MaxWidth/MaxHeight sizing locally and text subtitle links resolve through tokenless DLNA VTT routes; graphical subtitles are deliberately not advertised until a renderer-specific image subtitle path is implemented and real renderer-specific image sizing behavior remains pending.',
      'SystemUpdateID is persisted in named configuration, restored into process state and covered across SQLite reopen; packaging now verifies restart policy and persistent state/cache/log directories, while full restart validation of an installed systemd unit remains pending.',
      ...
      'Renderer header detection, VLC HLS-first DIDL ordering and metadata-gated AVC MP4 AAC protocolInfo are implemented for Samsung/LG/Sony while generic clients keep direct-first tolerant DIDL; broader renderer-specific transcode policy and real-device profile validation remain pending.',
    ],
  };
}
