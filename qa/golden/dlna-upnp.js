...
function buildEvidence(result, discoveryEvidence, eventsEvidence, manualEvidence) {
  ...
  return {
    ...
    openRisks: deviceValidated ? [] : [
      ...
      `Run npm run golden:dlna:device-preflight -- --base-url http://<server-lan-ip>:8097 --item-id <media-item-id> --subtitle-index <text-subtitle-index> to validate descriptor/icon/SCPD/SOAP plus Browse/Search/media-route readiness, then add at least one passing DLNA device evidence JSON under ${deviceEvidence.directory}.`,
