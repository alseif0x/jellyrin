  ...
  return {
    ...
    evidence: passed
      ? [
          ...
          'Manual device evidence schema v3 requires the renderer/control-point run to prove iconList, thumbnail and tokenless subtitle route fetches in addition to discovery, Browse and playback; npm run golden:dlna:device-fixture creates a reproducible video/audio/subtitle fixture for that device test, and npm run golden:dlna:device-preflight prepares a server-side draft and validates descriptor/icon/SCPD, ContentDirectory Browse/Search, ConnectionManager and MediaReceiverRegistrar SOAP readiness before that device run, accepts absolute or relative descriptor icon URLs, and supports optional --item-id/--subtitle-index probes for thumbnail and subtitle route readiness.',
          deviceValidated
    ...
    openRisks: deviceValidated ? [] : [
      ...
      `Run npm run golden:dlna:device-fixture, add the generated fixture directory as a movies library, then run npm run golden:dlna:device-preflight -- --base-url http://<server-lan-ip>:8097 --item-id <media-item-id> --subtitle-index 2 before adding at least one passing DLNA device evidence JSON under ${deviceEvidence.directory}.`,
