...
async function main() {
  ...
  try {
    ...
    await completeWizard(upstreamUrl, 'E5 Upstream', 'e5upstream');
    await completeWizard(jellyrinUrl, 'E5 Jellyrin', 'e5admin');
    await Promise.all([
      waitForCompletedTarget(upstreamUrl, 'upstream'),
      waitForCompletedTarget(jellyrinUrl, 'jellyrin'),
    ]);

    await runChannelsGolden(upstreamUrl, jellyrinUrl);
  } finally {
    await Promise.all(children.map(stopChild));
    if (!process.env.JELLYRIN_KEEP_CHANNELS_RUNNER_TMP) {
      await fs.rm(root, { recursive: true, force: true }).catch(() => {});
    } else {
      console.error(`kept Channels runner temp dir: ${root}`);
    }
  }
}
...
async function runChannelsGolden(upstreamUrl, jellyrinUrl) {
  await new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [path.join(repoRoot, 'qa/golden/channels-providers.js')], {
      cwd: repoRoot,
      stdio: 'inherit',
      env: {
        ...process.env,
        ...
        JELLYFIN_ADMIN_USER: 'e5upstream',
        JELLYFIN_ADMIN_PASSWORD: adminPassword,
        JELLYRIN_ADMIN_USER: 'e5admin',
    ...
    child.on('exit', (code, signal) => {
      if (code === 0) {
        ...
        reject(new Error(`Channels golden exited with ${signal || code}`));
      }
    });
  });
}
