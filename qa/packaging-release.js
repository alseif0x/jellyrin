...
async function main() {
  ...
  const checks = [
    check('docker-release-build', dockerfile.includes('cargo build --locked --release -p jellyrin-server -p jellyrin-migrate') && dockerfile.includes('RUNTIME_IMAGE=debian:bookworm-slim@sha256:')),
    ...
    check('docker-stable-secret-reader-identity', dockerfile.includes('groupadd --gid 10001 jellyrin') && dockerfile.includes('useradd --uid 10001 --gid jellyrin')),
    ...
    check('compose-postgres-private-network', infrastructureCompose.includes('${POSTGRES_IMAGE:-postgres:') && infrastructureCompose.includes('internal: true')),
    check('compose-infrastructure-images-locked', infrastructureCompose.includes('postgres:17.10-bookworm@sha256:') && infrastructureCompose.includes('redis:7.2.14-bookworm@sha256:')),
    ...
    check('supply-chain-lock-present', supplyChainLock.includes('RUST_IMAGE=rust:1.93.0-bookworm@sha256:') && supplyChainLock.includes('SYFT_LINUX_AMD64_SHA256=') && supplyChainLock.includes('JELLYFIN_WEB_VERSION=10.11.11') && supplyChainLock.includes('JELLYFIN_WEB_COMMIT=35c0793ece3adbd247eab290ae1effab851f3d37') && supplyChainLock.includes('JELLYFIN_WEB_TARBALL_SHA256=1dd84a8bf4aaa90b12ca38e72e68a554826ab0ea28bb354bcc3212f579e0a337') && supplyChainLock.includes('JELLYFIN_WEB_SWIPER_VERSION=12.1.2') && supplyChainLock.includes('JELLYFIN_WEB_SWIPER_PATCH_COMMIT=3cb38a0ac319edfcbcd331e3818cc9f6dec3e334') && supplyChainLock.includes('JELLYFIN_WEB_SWIPER_PATCH_SHA256=76e80a084337162a24dba022760492fa52f102fbf88747ff895b7076fc17f4b4')),
