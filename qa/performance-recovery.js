...
async function main() {
  ...
  const checks = [
    ...
    check('search-hints-use-bounded-native-catalog-with-safe-fallback', extractFunction(api, 'async fn search_hints_catalog_result').includes('resolved_media_catalog_query_for_items') && extractFunction(api, 'async fn search_hints_catalog_result').includes('MediaItemCatalogSearchScope::SearchHintFields') && !extractFunction(api, 'async fn search_hints_catalog_result').includes('.media_items()') && dbLib.includes('fn push_sqlite_search_hint_metadata_filter') && postgresCatalog.includes('fn push_postgres_search_hint_metadata_filter') && dbLib.includes("'Album', 'AlbumName', 'AlbumArtist', 'AlbumArtists'") && postgresCatalog.includes("'SeriesName', 'Series', 'Artists'")),
    check('series-id-lookup-is-tv-scoped-with-inline-metadata', extractFunction(api, 'async fn series_name_for_id').includes('MediaCatalogStore::tv_series_lookup_candidates') && !extractFunction(api, 'async fn series_name_for_id').includes('.media_items()') && extractFunction(dbLib, 'pub async fn tv_series_lookup_candidates').includes("lower(item.collection_type) IN ('tvshows', 'tvshow', 'series')") && extractFunction(dbLib, 'pub async fn tv_series_lookup_candidates').includes('item.metadata_json') && extractFunction(postgresCatalog, 'pub async fn tv_series_lookup_candidates').includes("lower(item.collection_type) = ANY(ARRAY['tvshows', 'tvshow', 'series']::text[])") && extractFunction(postgresCatalog, 'pub async fn tv_series_lookup_candidates').includes('item.metadata')),
