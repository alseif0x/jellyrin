...
async function main() {
  ...
  const checks = [
    ...
    check('ffmpeg-no-shell', core.includes('FfmpegCommandSpec::new("ffmpeg", args)') && ((transcode.includes('Command::new(command.program())') && transcode.includes('.args(command.args())')) || (transcode.includes('Command::new(&command.program)') && transcode.includes('.args(&command.args)')))),
